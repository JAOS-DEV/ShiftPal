Pro rollout plan (Stripe + Firebase Extension)
...
- Goal: Add paid Pro with Stripe; keep costs down until testing is done.
...
- (Optional) Enable Custom Claims: set a claim (e.g., { pro: true }) on active subscription.
...
- Settings → Account (if pro): show a "Manage subscription" button that calls `openBillingPortal()`.
- Keep `PRICE_ID` in a config/env: `VITE_STRIPE_PRICE_ID` (fallback to window.**CONFIG** for GH Pages if needed).

4. Role sync options (pick one)
   A) Use extension’s Custom Claims only

- Rules check `request.auth.token.pro == true` (and `admin` for admins).
...
- Create a minimal Cloud Function (http or onCreate/Update subscription) to:
  - Set `users/{uid}/profile/user.role = "pro"` when subscription is active.
  - Optionally set `proUntil` based on Stripe period_end.
  - Remove pro when cancelled/expired.
- Optionally also set custom claims for rules.

5. Security rules tightening (later)

- Switch admin/pro checks to Custom Claims for stronger enforcement (still keep profile for UI).
...
- Pro user: all features unlocked; cloud sync ON works; CSV, tax/NI, multiple rates work.
- Downgrade: pro → free auto-normalizes (tax/NI off, rates trimmed to 1, cloud local) and one-time recovery remains available if cloud data exists.
...
- Grace period handling using `proUntil` and scheduled function to clean up.
